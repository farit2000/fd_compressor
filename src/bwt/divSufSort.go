...
//Взято со стороны, так как только при таком алгоритме сортировке, деграданция наименьшая
...
const (
	...
	_MASK_FFFF0000              = -65536
	_MASK_FF000000              = -16777216
	_MASK_0000FF00              = 65280
)
...
// Основная структура DivSufSort для вычисления массива суффиксов или BWT с использованием
// алгоритм, разработанный Ютой Мори.
...
// NewDivSufSort создает новый экземпляр DivSufSort
...
// ComputeSuffixArray генерирует массив суффиксов для заданных данных и возвращает его
// в срезе 'sa'.
...
func (this *DivSufSort) constructSuffixArray(bucketA, bucketB []int32, n, m int32) {
	if m > 0 {
		for c1 := 254; c1 >= 0; c1-- {
			...
			// Сканируем массив суффиксов справа налево.
	...
	// Сканируем массив суффиксов слева направо.
	for i := int32(0); i < n; i++ {
		s := this.sa[i]
...
// ComputeBWT генерирует BWT для заданных данных и возвращает его
// в срезе 'sa'.
...
func (this *DivSufSort) sortTypeBstar(bucketA, bucketB []int32, n int32) int32 {
	...
	// Подсчитываем количество вхождений первых одного или двух символов каждого
	// введите суффикс A, B и B *. Кроме того, сохраните начальную позицию всех
	// суффиксы типа B * в массив SA.
	for i := n - 1; i >= 0; {
		c1 := c0
	...
	// Суффикс типа B * лексикографически меньше, чем суффикс типа B, который
	// начинается с тех же первых двух символов.

	// Вычислить индекс начальной / конечной точки каждого сегмента.
	for i, j := int32(0), int32(0); x0 < 256; x0++ {
		...
		for x1 := x0 + 1; x1 < 256; x1++ {
			j += bucketB[idx+x1]
			bucketB[idx+x1] = j // конечная точка
			i += bucketB[(x1<<8)+x0]
		}
	}

	if m > 0 {
		// Сортируем суффиксы типа B * по их первым двум символам.
		pab := n - m
		...
		// Сортируем подстроки типа B * с помощью ssSort.
		bufSize := n - m - m
		x0 = 254
		...
		// Вычислить ранги подстрок типа B *.
		for i := m - 1; i >= 0; i-- {
			if arr[i] >= 0 {
				j := i
		...
		// Создайте массив обратных суффиксов для суффиксов типа B * с помощью trSort.
		this.trSort(m, 1)

		// Установить порядок сортировки суффиксов типа B *.
		c0 = this.buffer[n-1]
		var c1 byte
		...
		for x0 = 254; x0 >= 0; x0-- {
			...
			for x1 := 255; x1 > x0; x1-- {
				...
				// Перемещаем все суффиксы типа B * в правильную позицию.
				// Обычно очень небольшое количество копий
			...
			bucketB[x2+x0+1] = i - bucketB[x2+x0] + 1 // начальная точка
			bucketB[x2+x0] = i                        // конечная точка
		}
	}
...
func (this *DivSufSort) ssMultiKeyIntroSort(pa, first, last, depth int32) {
	...
	for {
		...
		// Создаем псевдонимы срезов
		// ПРИМЕЧАНИЕ: buf1 может заменить this.buffer только тогда, когда индекс гарантирован
		// быть положительным или нулевым (не в шаблоне вроде this.buffer [xyz-1]) !!!
...
// Тандемная повторная сортировка
...
// Стек предварительно выделенных элементов
type stack struct {
	elts  []stackElement
	index int32
}
